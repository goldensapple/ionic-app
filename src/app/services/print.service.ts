...
export class PrintService {
  ...
  constructor(private cartService: CartService, public paymentService: PaymentService, public http: HttpClient, public storage: Storage, public router: Router, public cashService: CashService, public activeRoute: ActivatedRoute, public couponService: CouponService, public apiService: ApiService, public storageService: StorageService) {}
  ...
  getCanvas(){
    return new Promise( (resolve) => {
      ...
      image1.src = this.receiptJson[0].header_image;
  ...
  getCanvasFooter(){
    return new Promise ( (resolve) => {
      ...
      image2.src = this.receiptJson[0].footer_image;
  ...
  setLocalPrinter(){
    ...
    this.ipAddress = '192.168.1.8';
    console.log('local printer ip address set: ', this.ipAddress);
    this.port = 8008;
    ...
    // return new Promise (resolve => {
      ...
      if (this.deviceID === 'local_printer'){
        ...
        this.printer.onreceive = async (res) => {
          ...
          // if (sEmpty) {this.alertToPrinter(sEmpty); }
          // if (sNearEmpty) {this.alertToPrinter(sNearEmpty); }
  ...
  getTotalWithTax(){
    return this.paymentService.totalPriceWithTax;
  }
  ...
  async receiptDesign(){
    ...
    this.receiptJson = await this.apiService.getMasterJson("RECEIPT_CONFIG", this.shopId);

    console.log('this.receiptJson', this.receiptJson);
    ...
    for (const product of this.apiService.products){
      // 半角スペース除去
      const productName = product.name.replace(/\s/g, '');
      const nameLength = productName.length * 2;
      ...
      this.priceLength = product.price.toLocaleString().length;
      this.priceAndAmountLength = this.priceLength + ('@' + 'x' + product.amount).length;
      ...
      this.printer.addText('@' + product.price.toLocaleString() + 'x' + product.amount);
      this.printer.addText(' '.repeat(15 - this.priceAndAmountLength));
    ...
    if(this.receiptJson[0].footer_image != ''){
  ...
  async receiptRoom(){
    ...
    this.printer.addTextDouble(true, true);
    const roomName = this.apiService.chosenRoom.room_name;
    const startTime = this.apiService.startTime;
    const endTime = this.apiService.endTime;

    const roomLabel = 'お部屋';
    const startLabel = 'ご入室時間';
    const endLabel = 'ご退室予定時間';


    this.printer.addText(roomLabel.padEnd(16 - (roomLabel.length)));
    this.printer.addText(String(roomName + '号室').padStart(8) + '\n');

    this.printer.addText(startLabel.padEnd(16 - (startLabel.length)));
    this.printer.addText(String(startTime).padStart(8) + '\n');

    this.printer.addText(endLabel.padEnd(16 - (endLabel.length)));
    this.printer.addText(String(endTime).padStart(8) + '\n');
